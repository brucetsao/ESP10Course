#include <Arduino.h>
#include <U8g2lib.h>    //Oled 128_16/32/64/96/128最底層函式庫
#include <SPI.h>    // SPI 最底層函式庫
#include <Wire.h>   //I2C最底層函式庫
...
U8G2_SSD1306_128X32_UNIVISION_F_HW_I2C u8g2(U8G2_R0);   //設定本次使用12832解析度的顯示器
char oledword[100] ;    //暫存char
...
ss: UTF-8 encoded text.
 ...
 void ShowFloatonOled(int x, int y, float ss) //在(x,y)位置顯示文字、數字
{
    sprintf(oledword,"%f",ss) ;   //格式化列印
 
    _TPrint(x,y,oledword);      //
} 
void ShowLongonOled(int x, int y, long ss)  //在(x,y)位置顯示文字、數字
{
    sprintf(oledword,"%d",ss) ;   //格式化列印
 
    _TPrint(x,y,oledword);      //
} 
void ShowIntonOled(int x, int y, int ss)    //在(x,y)位置顯示文字、數字
{
    sprintf(oledword,"%d",ss) ;   //格式化列印
 
    _TPrint(x,y,oledword);      //
} 
void ShowStringonOled(int x, int y, String ss)  //在(x,y)位置顯示文字、數字
{
    sprintf(oledword,"%s",ss) ;   //格式化列印
     _TPrint(x,y,oledword);      //
     Serial.println(oledword) ;
}
 void ShowMAConLCD(String s1)   //顯示MAC Address基本資訊
{
      Serial.println(s1) ;
   ShowStringonOled(3,10,s1);      //顯示MAC Address基本資訊
}
 void ShowTestonLCD(String s1)   //顯示測試號碼
{
  Serial.println(s1) ;
   ShowStringonOled(1,31,s1);      //顯示測試號碼
}
 void ShowTestIntonLCD(int s1)   //顯示測試號碼
{
  //在(x,y)位置顯示文字、數字
   ShowIntonOled(1,31,s1);      //顯示測試號碼
}
 void ShowTestLongonLCD(long s1)   //顯示測試號碼
{
    //在(x,y)位置顯示文字、數字
   ShowLongonOled(1,31,s1);      //顯示測試號碼
}
