...
y1data=[]
y2data=[]
#http://localhost:8088/fcu/dhtdata/dht2jsonwithday.php?MAC=CC50E3B5BB20&start=20220101&end=20220131
#可以用下面SQL語法，進行擷取筆數
# select  MAC, count(*) as cnt from dhtdata  where 1 group by MAC  order by MAC

url1 = "http://localhost:8088/fcu/dhtdata/dht2jsonwithday.php?MAC=%s&start=%s&end=%s"
...
plt.plot(xdata, y1data, color='c', label='溫度')          # 設定青色cyan            
plt.title("每日平均溫度", fontsize=24)
plt.xlabel("日期", fontsize=14)
plt.ylabel("溫度", fontsize=14)
#plt.plot(xdata, y2data, color='r')          # 設定紅色red
plt.savefig('getLocalDHTData_day2.pdf')
plt.show()