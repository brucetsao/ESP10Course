<?php     
//http://nuk.arduino.org.tw:8888/dhtdata/dht2jsonwithdayhourlist.php?MAC=E89F6DE8F3BC&start=20200101&end=20221231

   //http://nuk.arduino.org.tw:8888/ Host URL
 ...
 $qry1 = "select MAC,left(systime,10) as systime FROM nukiot.dhtData where MAC = '%s' and systime >= '%s' and systime <= '%s' group by left(systime,10)  order by systime asc;" ;		//將dhtdata的資料找出來
 ...
 while($row = mysqli_fetch_array($result)) 
	 {
		 ...
		 $subqry="select temperature, humidity from nukiot.dhtData where MAC='%s' and left(systime,10) = '%s' order by systime asc;";
