...
<?php
	...
	$qrystr="select mac as MAC, count(*) as cnt from OW_curent group by MAC " ;		//將dhtdata的資料找出來
  ...
  if($result!==FALSE){
	 while($row = mysql_fetch_array($result)) 
	 {
			...
			array_push($d01, $row["cnt"]);	// array_push(某個陣列名稱,加入的陣列的內容
		}// 會跳下一列資料
