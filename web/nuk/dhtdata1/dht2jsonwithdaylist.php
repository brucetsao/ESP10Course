<?php     
   //http://140.134.25.43:8088/fcu/dhtdata/dht2jsonwithdaylist.php?MAC=CC50E3B5BB20&start=20210101&end=20211231

	   //http://140.134.25.43:8088/ Host URL
	 //Restful API 的程式：：/fcu/dhtdata/dht2jsonwithdaylist.php?MAC=3C71BFFD882C&start=20210101&end=20211231  
...
//		$qry1 = "select MAC,left(systime,8) as systime, avg(temperature) as temperature, avg(humidity) as humidity FROM fcu.dhtData where MAC = '%s' and systime >= '%s' and systime <= '%s' group by left(systime,8)  order by systime asc;" ;		//將dhtdata的資料找出來
		$qry1 = "select MAC,left(systime,8) as systime FROM fcu.dhtData where MAC = '%s' and systime >= '%s' and systime <= '%s' group by left(systime,8)  order by systime asc;" ;		//將dhtdata的資料找出來
	 ...
	 while($row = mysqli_fetch_array($result)) 
		 {
			 ...
			 $subqry="select temperature,humidity from fcu.dhtData where MAC='%s' and left(systime,8) = '%s' order by systime asc;";
