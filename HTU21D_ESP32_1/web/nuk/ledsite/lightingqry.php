...
<?php	
$cnt = 1 ;
$goddata = "<td><p><div align = 'center'><img src='/fcu/images/god/%d.jpg' width='150' height='195' /></div></p><div align = 'center'><p>%s</p></div><p>香主名稱:%s</p><p>香主生辰:%s</p><p></p></td>\n" ;
...
if($result!==FALSE)	//判斷是否有資料
{
 while($row = mysqli_fetch_array($result)) //每次抓一筆資料，有抓到資料傳回true， 沒有傳回false
 {
	...
	$goddatastr = sprintf($goddata,$row0["godtype"],$row["no"],$row["donater"],$row["dbirthday"]) ;
	echo $goddatastr  ;
		if (($cnt % 4)== 0)
		{
			echo "</tr>" ;
