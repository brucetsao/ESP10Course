...
<?php
    ...
    echo "<form id='form' name='form' method='POST' action='receivecoutyselectpro.php'>" ;
	...
	if ($count >0)
	{
       ...
       $rowdata = "<tr><td><input type='checkbox' name='selectdata[]' value='%s'  group='1'>%s</td><td><div id = 'a%d'>%s</div></td><td>%s</td></tr>\n" ;
       for( $i=0; $i < count($d00) ; $i=$i +1 )
       		{
            $rowstr =  sprintf($rowdata,$d00[$i],$i+1,$i+1,$d00[$i],$d01[$i]) ;
            echo  $rowstr  ;
