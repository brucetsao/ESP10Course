...
#include "RFIDLib.h"  // 溫溼度專用模組 共用函數
...
void loop() 
{
    ...
    RKey = ReadRFID(); //讀取HTU21D 溫溼度感測器之溫度

    Serial.print("Key: ");               //印出 "Key: "
    Serial.print(RKey);                   //印出 RKey變數內容
    Serial.println("");                //印出 "換行"
    _clearOled(); //清除螢幕
    printRFIDonOled(RKey) ;
